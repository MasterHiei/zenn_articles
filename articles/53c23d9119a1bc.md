...
なるほど、それぞれは6文字以上の文字列と、その長さをペアにして出力していますね。
どう思うでしょうか？
...
`Either`を使用したことによって、以下のコードをコンパイルするとエラーが発生します。
...
おっと、`parse()`の結果は`Right`でも`Left`でも、両方を処理しないといけないですね！
つまり、今まで実行時しか発生しないエラーを、コンパイル時に発生させるようにしたってことですね！

## `Either`と`switch`

`Either<L, R>`は`sealed class`なので、`match`以外に通常の`switch文`を使うことも可能です。
